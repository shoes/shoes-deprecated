...
static gboolean shoes_button_gtk_clicked(GtkButton *button, gpointer data) {
    VALUE self = (VALUE)data;
...
SHOES_CONTROL_REF shoes_native_text_edit_box(VALUE self, shoes_canvas *canvas, shoes_place *place, VALUE attr, char *msg) {
...
VALUE shoes_native_text_edit_box_get_text(SHOES_CONTROL_REF ref) {
    GtkWidget *textview;
    GTK_CHILD(textview, ref);
...
void shoes_native_text_edit_box_set_text(SHOES_CONTROL_REF ref, char *msg) {
    GtkWidget *textview;
    GTK_CHILD(textview, ref);
    GtkTextBuffer *buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(textview));
    gtk_text_buffer_set_text(buffer, _(msg), -1);
...
VALUE shoes_native_text_edit_box_append(SHOES_CONTROL_REF ref, char *msg) {
    GtkWidget *textview;
    GTK_CHILD(textview, ref);
    GtkTextBuffer *buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(textview));
    GtkTextIter begin, end;
