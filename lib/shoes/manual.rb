...
# manual from the command line (i.e. executing bin/shoes). The Shoes::Manual
# module is mostly defined in lib/shoes/help.rb
#
# FIXME: Collect all Shoes::Manual code in one place.
module Shoes::Manual
  def self.p(str, path)
    str.gsub(/\n+\s*/, " ").
      gsub(/&/, '&amp;').gsub(/>/, '&gt;').gsub(/>/, '&lt;').gsub(/"/, '&quot;').
  ...
  def self.as format, *args
    ...
    when :shoes
      ...
      html_bits = proc do
        proc do |sym, text|
        case sym when :intro
          div.intro { p { self << man.p(text, dir) } }
        when :code
        ...
        #   index_page
        when :list
          ul { text.each { |x| li { self << man.p(x, dir) } } }
        when :samples
          ...
          send(TITLES[sym] || :p) { self << man.p(text, dir) }
        end
        end
      end
