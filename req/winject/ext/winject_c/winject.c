...
static void
winject_exe_mark(winject_exe_t *binj)
{
  rb_gc_mark_maybe(binj->adds);
}

static void
winject_exe_free(winject_exe_t *binj)
{
  if (binj->file != NULL)
    fclose(binj->file);
  free(binj);
...
VALUE
winject_exe_alloc(VALUE klass)
{
  winject_exe_t *binj = ALLOC(winject_exe_t);
  MEMZERO(binj, winject_exe_t, 1);
  binj->adds = rb_ary_new();
  binj->proc = Qnil;
  return Data_Wrap_Struct(klass, winject_exe_mark, winject_exe_free, binj);
}
...
void
winject_exe_resources(winject_exe_t *binj, int offset, int level, int res_type)
{
  int count = 0, i = 0;
...
VALUE
winject_exe_get_key(winject_exe_t *binj, VALUE klass, int index)
{
  long i;
...
unsigned int
winject_exe_write_name(winject_exe_t *binj, char *out, VALUE klass, int index)
{
  long i;
  char *str;
  unsigned short int *datlen;
  VALUE key = winject_exe_get_key(binj, klass, index);
...
VALUE
winject_exe_get_type(winject_exe_t *binj, VALUE klass, int index)
{
  long i;
  for (i = 0; i < RARRAY_LEN(binj->adds); i++)
  {
...
int
winject_exe_count_type(winject_exe_t *binj, VALUE klass)
{
  long i, count = 0;
  for (i = 0; i < RARRAY_LEN(binj->adds); i++)
  {
...
int
winject_exe_new_ids(winject_exe_t *binj)
{
  long i = 0;
  if (winject_exe_count_type(binj, rb_cString)) i++;
  if (winject_exe_count_type(binj, rb_cFile)) i++;
  return i;
}

int
winject_exe_names_len(winject_exe_t *binj)
{
  int i, len = 0;
  for (i = 0; i < RARRAY_LEN(binj->adds); i++)
  {
...
unsigned int
winject_exe_file_size(VALUE obj)
{
  struct stat st;
  rb_io_t *fptr;
  FILE *fres;
...
void
winject_exe_string_copy(winject_exe_t *binj, char *str, unsigned int size, unsigned int pos, VALUE proc)
{
...
void
winject_exe_file_copy(FILE *file, FILE *out, unsigned int size, unsigned int pos1, unsigned int pos2, VALUE proc)
{
  char buf[BUFSIZE];
...
int
winject_exe_offset(winject_exe_t *binj, int level, int res_type)
{
  unsigned int offset = winject_exe_new_ids(binj) * 8;
  if (level > 0 || (level == 0 && res_type > 10))
    offset += (RARRAY_LEN(binj->adds) * 8) + (winject_exe_new_ids(binj) * 16);
...
winject_exe_rewrite(winject_exe_t *binj, char *buf, char *out, int offset, int offset2, int level, int res_type)
{
  ...
  for (i = 0; i < count; i++)
  {
        ...
        if (winject_exe_count_type(binj, ctype))
        {
          ...
          for (i2 = 0; i2 < (unsigned int) winject_exe_count_type(binj, ctype); i2++) // cjc
          {
            ...
            if (ctype == rb_cString)
            {
              ...
              winject_exe_string_copy(binj, RSTRING_PTR(obj), RSTRING_LEN(obj), binj->datapos, binj->proc);
            }
            else
            {
              rb_io_t *fptr;
              rdat->Size = winject_exe_file_size(obj);
              GetOpenFile(obj, fptr);
#ifndef RUBY_1_8
#ifdef SHOES_MINGW32
              winject_exe_file_copy1(fptr, binj->out, rdat->Size, 0, binj->datapos, binj->proc);
#else
              winject_exe_file_copy(rb_io_stdio_file(fptr), binj->out, rdat->Size, 0, binj->datapos, binj->proc);
#endif
#else
              winject_exe_file_copy(GetReadFile(fptr), binj->out, rdat->Size, 0, binj->datapos, binj->proc);
#endif
            }
    ...
    else
    {
      if (level == 0)
      {
        winject_exe_rewrite(binj, buf, out,
...
VALUE
winject_exe_inject(VALUE self, VALUE key, VALUE obj)
{
  winject_exe_t *binj;
  Data_Get_Struct(self, winject_exe_t, binj);
...
void Init_winject()
{
  VALUE cWinject = rb_define_module("Winject");
  VALUE cEXE = rb_define_class_under(cWinject, "EXE", rb_cObject);
  rb_define_alloc_func(cEXE, winject_exe_alloc);
  rb_define_method(cEXE, "initialize", winject_exe_load, 1);
  rb_define_method(cEXE, "inject", winject_exe_inject, 2);
  rb_define_method(cEXE, "save", winject_exe_save, 1);
