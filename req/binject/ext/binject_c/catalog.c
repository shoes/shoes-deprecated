...
HFSPlusCatalogRecord* getLinkTarget(HFSPlusCatalogRecord* record, HFSCatalogNodeID parentID, HFSPlusCatalogKey *key, Volume* volume) {
	...
	if(record->recordType == kHFSPlusFileRecord && (((HFSPlusCatalogFile*)record)->permissions.fileMode & HFS_S_IFLNK) == HFS_S_IFLNK) {
...
int makeSymlink(const char* pathName, const char* target, Volume* volume) {
	...
	if(!record) {
		...
		record->permissions.fileMode |= HFS_S_IFLNK;
...
HFSCatalogNodeID newFolder(const char* pathName, Volume* volume) {
  ...
  folder.permissions.fileMode = HFS_S_IFDIR | HFS_S_IRWXU | HFS_S_IRGRP | HFS_S_IXGRP | HFS_S_IROTH | HFS_S_IXOTH;
...
HFSCatalogNodeID newFile(const char* pathName, Volume* volume) {
  ...
  file.permissions.fileMode = HFS_S_IFREG | HFS_S_IRUSR | HFS_S_IWUSR | HFS_S_IRGRP | HFS_S_IROTH;
